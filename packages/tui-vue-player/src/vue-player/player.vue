  ...
  <div class="audio-player-container" :class="{ mini }">
    <div class="audio-player-warper">
      ...
      <div class="audio-player-control-btn">
        <template v-if="!error">
          <svg viewBox="-9 0 32 32" class="audio-player-control-icon" v-if="!(isIE ? playing : isPlaying)" @click="togglePlay">
      ...
      <div class="audio-player-warper-content" v-if="!mini">
        ...
        <div class="audio-player-control" v-else>
          ...
          <svg viewBox="0 0 32 32" class="audio-player-control-icon volume" v-if="isIE ? muted : isMuted" @click="toggleMute">
...
export default {
  ...
  methods: {
    ...
    changeTime (rate) {
      if (this.isIE) {
        this.player && (this.player.controls.currentPosition = this.player.currentMedia.duration * rate)
      } else {
        this.player && this.player.changeTime(this.durationNum * rate)
      }
    },
    /**
     * 调节音量
    ...
    changeVolume (volume) {
      if (this.isIE) {
        this.player.settings.volume = volume * 100
        this.mute(volume === 0)
      } else {
    ...
    changeMedia (media) {
      if (media === this.value) {
        ...
        if (this.isIE) {
          ...
          this.value = media
          // 初始化新的音频，保留静音和音量
          this.player.initPlayer(media.src, this.isMuted ? 0 : this.volume)
          this.togglePlay()
        }
      }
    },
